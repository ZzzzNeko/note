...
```js
// 模拟 is
// 解决 -0 与 +0 ，NaN 与 NaN 之间的判断 (认为第一个不同，第二个相同)
Object.mockIs = function (a, b) {
  if (a === b) {
    return a !== 0 || 1 / a === 1 / b;
  } else {
    return a !== a && b !== b;
...
```js
// 模拟 create
Object.mockCreate = function (proto, propertiesObject) {
...
// Child.prototype = Object.create(Parent.prototype); //简写?
...
function mokeNew(klass, ...args) {
  ...
  return ["object", "function"].includes(typeof ret) ? ret : obj;
}
```
...
// 模拟call
// NOTE: 无法完全实现 call，调用时上下文会传递暂存属性(这里使用 _fn，es6可以使用 Symbol)
...
// 模拟apply
// NOTE: 无法完全实现 apply，调用时上下文会传递暂存属性(这里使用 _fn，es6可以使用 Symbol)
